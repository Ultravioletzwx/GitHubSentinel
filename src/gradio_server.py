...
from towardsdatascience_client import TowardsDataScienceClient
...
towardsDS_client = TowardsDataScienceClient(proxy_address='http://127.0.0.1:7890')
...
def generate_tds_report(model_type, model_name,section_name):
    config.llm_model_type = model_type
    ...
    report, report_file_path = report_generator.generate_towardsdatascience_report(markdown_file_path)
    return report, report_file_path

# 定义一个回调函数，用于根据 Radio 组件的选择返回不同的 Dropdown 选项
def update_model_list(model_type):
    if model_type == "openai":
        return gr.Dropdown(choices=["gpt-4o", "gpt-4o-mini","gpt-4o-2024-08-06","o1-mini"], label="选择模型")
    elif model_type == "ollama":
        return gr.Dropdown(choices=["llama3.1:8b", "qwen2:latest"], label="选择模型")
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("GitHub 项目进展"):
        ...
        # 创建 Dropdown 组件
        model_name = gr.Dropdown(choices=["gpt-4o", "gpt-4o-mini","gpt-4o-2024-08-06","o1-mini"], label="选择模型")
    ...
    # 创建 Towards Data Science 文章
    with gr.Tab("Towards Data Science 最新文章和趋势"):
        gr.Markdown("## Towards Data Science 最新文章和趋势")  # 添加小标题
        ...
        # 创建 Dropdown 组件
        model_name = gr.Dropdown(choices=["gpt-4o", "gpt-4o-mini", "gpt-4o-2024-08-06", "o1-mini"],
                                 label="选择模型")

        # 创建订阅列表的 Dropdown 组件
        section_name = gr.Dropdown(["latest","trending"], label="获取最新趋势文章",
                                        info="可选 [最新] 或者 [趋势]")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_tds_report, inputs=[model_type, model_name, section_name],
